 ...
 */
typedef struct {
	BS2_DATETIME			date;		///< 4 bytes
	BS2_HOLIDAY_RECURRENCE	recurrence;		///< 1 bytes
...
typedef struct {
	...
	char					name[BS2_MAX_HOLIDAY_GROUP_NAME_LEN];		///< 144 bytes
	uint8_t					numHolidays;		///< 1 byte
	uint8_t					reserved[3];		///< 3 bytes (packing)
	BS2Holiday				holidays[BS2_MAX_HOLIDAYS_PER_GROUP];		///< 8 * 128 bytes
} BS2HolidayGroup;
