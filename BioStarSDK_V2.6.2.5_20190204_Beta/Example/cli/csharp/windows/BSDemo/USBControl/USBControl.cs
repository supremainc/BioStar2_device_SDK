    ...
    public class USBControl : FunctionModule
    {
        ...
        void getLog(IntPtr sdkContext, UInt32 deviceID, bool isMasterDevice)
        {
            ...
            do
            {
                ...
                BS2ErrorCode result = (BS2ErrorCode)API.BS2_GetLogFromDir(sdkContext, strDir, lastEventId, amount, out outEventLogObjs, out outNumEventLogs);
        ...
        void getFilteredLog(IntPtr sdkContext, UInt32 deviceID, bool isMasterDevice)
        {
            ...
            BS2ErrorCode result = (BS2ErrorCode)API.BS2_GetFilteredLogFromDir(sdkContext, strDir, uid, eventCode, start, end, tnaKey, out outEventLogObjs, out outNumEventLogs);
            if (result != BS2ErrorCode.BS_SDK_SUCCESS)
            {
                Console.WriteLine("Got error({0}).", result);
            }
        ...
        void getLogBlob(IntPtr sdkContext, UInt32 deviceID, bool isMasterDevice)
        {
            ...
            {
                outEventLogObjs = IntPtr.Zero;
                BS2ErrorCode result = (BS2ErrorCode)API.BS2_GetLogBlobFromDir(sdkContext, strDir, (ushort)BS2EventMaskEnum.ALL, lastEventId, amount, out outEventLogObjs, out outNumEventLogs);
                if (result != BS2ErrorCode.BS_SDK_SUCCESS)
                {
                    Console.WriteLine("Got error({0}).", result);
                    break;
        ...
        public void getDatabaseInfo(IntPtr sdkContext, UInt32 deviceID, bool isMasterDevice)
        {
            ...
            BS2ErrorCode result = (BS2ErrorCode)API.BS2_GetUserDatabaseInfoFromDir(sdkContext, strDir, out numUsers, out numCards, out numFingers, out numFaces, cbIsAcceptableUserID);
            if (result == BS2ErrorCode.BS_SDK_SUCCESS)
            {
            ...
            if (result == BS2ErrorCode.BS_SDK_SUCCESS)
            {
                if (numUserIds > 0)
                {
                    ...
                    for (UInt32 idx = 0; idx < numUserIds;)
                    {
                        ...
                        result = (BS2ErrorCode)API.BS2_GetUserInfosFromDir(sdkContext, strDir, curUidObjs, available, userBlobs);
                        if (result == BS2ErrorCode.BS_SDK_SUCCESS)
                        {
            ...
            if (result == BS2ErrorCode.BS_SDK_SUCCESS)
            {
                if (numUserIds > 0)
                {
                    ...
                    for (UInt32 idx = 0; idx < numUserIds;)
                    {
                        ...
                        result = (BS2ErrorCode)API.BS2_GetUserInfosExFromDir(sdkContext, strDir, curUidObjs, available, userBlobs);
                        if (result == BS2ErrorCode.BS_SDK_SUCCESS)
                        {
                            for (UInt32 loop = 0; loop < available; ++loop)
                            {
